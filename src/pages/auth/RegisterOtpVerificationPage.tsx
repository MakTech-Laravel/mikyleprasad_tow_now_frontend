...
export default function RegisterOtpVerificationPage() {
  ...
  const role = locationState?.role ?? params.get('role') ?? 'user';
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    setLoading(true);
    try {
      const res = await request.post<PassportLoginPayload>('/otp/register/verify', {
        email,
        code: data.code,
  ...
  async function resendCode() {
    ...
    setResending(true);
    try {
      const res = await request.post<OtpResponse>('/otp/register/resend', { email });
        ...
        <Card className="w-full max-w-md border-primary bg-input/20">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3 text-center text-sm text-muted-foreground">
              ...
              <p>
                <Link to={`/register?role=${role}`} className="underline-offset-4 hover:underline">
