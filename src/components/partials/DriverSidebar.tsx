...
import { BookOpen, LogOut, Star, TrendingUp, User, X } from 'lucide-react';
...
// ── Nav items — every item now has an icon ─────────────────────────────────────

const items: NavItem[] = [
  { icon: TrendingUp, to: '/driver-app', label: 'Dashboard', end: true },
  { icon: BookOpen, to: '/driver-app/bookings', label: 'Bookings' },
  { icon: User, to: '/driver-app/profile', label: 'Profile' },
  { icon: Star, to: '/driver-app/review', label: 'Review' },
];
...
// ── Logout button — reused in both desktop + mobile ────────────────────────────
...
export function DriverSidebar({ open, onClose }: DriverSidebarProps) {
  ...
  return (
    <>
      {/* ── Desktop sidebar (always in flow, hidden below md) ──────────────── */}
      ...
      >
        ...
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-border px-4">
          <span className="font-montserrat text-sm font-bold">Driver Menu</span>
        ...
        {/* Logout — now present in mobile drawer too */}
        <LogoutButton onClick={handleLogout} />
      </motion.aside>
    </>
  );
